...
export function ExampleVideos({ onPick, busy }) {
  ...
  return (
    <section className="panel example-videos" aria-labelledby="examples-heading">
      <h2 id="examples-heading">Ready to play</h2>
      <p className="help" id="examples-help">
        {videos.length} {videos.length === 1 ? 'video has' : 'videos have'} already been described on
        this server. They start straight away, with no processing wait.
      </p>
          ...
          <li key={video.videoId}>
            <button
              ...
              aria-label={`${video.title}${video.channel ? `, by ${video.channel}` : ''}${
                ...
                video.descriptions ? `, ${video.descriptions} audio descriptions` : ''
              }. Ready to play now.`}
            >
              ...
              <span className="result-body">
                ...
                <span className="result-meta" aria-hidden="true">
                  ...
                  {video.duration ? formatTime(video.duration) : ''}
                  {video.descriptions ? ` · ${video.descriptions} descriptions` : ''}
                </span>
              </span>
