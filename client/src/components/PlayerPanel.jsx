...
export const PlayerPanel = forwardRef(function PlayerPanel(
  ...
  const position = `${formatTime(currentTime)} of ${formatTime(duration)}`;

  return (
    <section className="panel player" aria-labelledby="player-heading">
      <div className="panel-title-row">
        <h2 id="player-heading">{title || 'Video'}</h2>
        {language && (
      ...
      <div className="transport" role="group" aria-label="Playback controls">
        ...
        >
          ...
          <span className="t-label">{muted ? 'Unmute' : 'Mute'}</span>
        </button>
        ...
        >
          ...
          <span className="t-label">Descriptions {descriptionsEnabled ? 'on' : 'off'}</span>
        </button>
        ...
        >
          ...
          <span className="t-label">Skip speech</span>
        </button>

        <button type="button" className="transport-btn" onClick={onRepeat} aria-keyshortcuts="R">
          <Icon name="replay" size={22} />
          <span className="t-label">Replay last</span>
        </button>
      </div>

      {/* A live, non-intrusive banner while VisionBridge is talking, so the
      ...
      {speaking && (
        <p className="speaking-banner" aria-hidden="true">
          ...
          <span>
            VisionBridge is speaking — press <kbd>S</kbd> to skip, <kbd>R</kbd> to replay.
          </span>
      ...
      <div className="seek">
        <label htmlFor="seek-slider">Position in video</label>
        <div className="seek-row">
          <input
            ...
            aria-valuetext={speakTime(currentTime)}
          />
