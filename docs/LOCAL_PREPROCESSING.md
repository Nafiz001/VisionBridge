...
- **Playback never uses the downloaded video file.** VisionBridge embeds the
...
`/opt/visionbridge/.cache/` on the VM:
...
4. **Copy them to the VM** — see the SSH section below for connecting, then:
   ```bash
   scp -i visionbridge_key.pem .cache/videos/VIDEO_ID.* \
     visionbridge@visionbridge.centralindia.cloudapp.azure.com:/opt/visionbridge/.cache/videos/

   scp -i visionbridge_key.pem \
     ...
     visionbridge@visionbridge.centralindia.cloudapp.azure.com:/opt/visionbridge/.cache/data/
   ```
5. **Verify** — load the video on the live deployed app
   (https://visionbridge-ten.vercel.app) and confirm it plays with
...
- **Host**: `visionbridge.centralindia.cloudapp.azure.com`
- **User**: `visionbridge`
- **Key**: `visionbridge_key.pem` (repo root — **gitignored**, never commit it)

```bash
ssh -i visionbridge_key.pem visionbridge@visionbridge.centralindia.cloudapp.azure.com
```
...
az vm user update \
  --resource-group VISIONBRIDGE_GROUP \
  --name visionbridge \
  --username visionbridge \
...
sudo journalctl -u visionbridge -f

# disk usage — plenty of headroom (25GB free of 29GB as of this writing)
df -h /
du -sh /opt/visionbridge/.cache/*

# restart after any .env or code change
cd /opt/visionbridge && git pull -q && sudo systemctl restart visionbridge
```
...
This only covers videos that have actually been pre-processed. A judge
...
pool at that moment). For a hackathon submission where judges are mainly
evaluating the demo/case-study videos named in the writeup, that's usually
the realistic usage pattern — but it's worth pre-processing a slightly wider
set than the bare minimum, in case a judge tries something adjacent.
