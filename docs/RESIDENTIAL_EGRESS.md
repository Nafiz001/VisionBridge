...
```bash
# /opt/visionbridge/.env
YTDLP_PROXY=socks5://127.0.0.1:1080
```

Then `sudo systemctl restart visionbridge`.
...
```bash
ssh -N -R 1080 \
  -i visionbridge_key.pem \
  -o ExitOnForwardFailure=yes \
  -o ServerAliveInterval=30 \
  -o ServerAliveCountMax=3 \
  visionbridge@visionbridge.centralindia.cloudapp.azure.com
```
...
# save as tunnel.ps1
while ($true) {
  ssh -N -R 1080 -i D:\VisionBridge\visionbridge_key.pem `
...
```powershell
schtasks /create /tn VisionBridgeTunnel /sc onlogon /rl highest `
  /tr "powershell -WindowStyle Hidden -ExecutionPolicy Bypass -File D:\VisionBridge\tunnel.ps1"
```
