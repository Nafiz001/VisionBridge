/**
 * Gemma client — the ONLY generative model in VisionBridge.
 *
 ...
 * checkpoint, which is text-only. VisionBridge cannot work without vision, so
  ...
  const gemmaModels = available
    ...
    .filter((id) => {
      try {
        ...
        return false; // Not a Gemma model — VisionBridge will not consider it.
      }
    });
  ...
  if (!selected) {
    throw new AppError(
      'No Gemma model with vision support is available to this API key. ' +
        ...
        'VisionBridge will not substitute a non-Gemma model.',
      503,
      'no_gemma_model',
    );
  }
