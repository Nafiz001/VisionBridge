...
export async function generateTimeline({ videoId, options = {}, refresh = false, onProgress }) {
  ...
  // The narration language decides the output language, which in turn keys the
  // timeline cache — so resolve it (and check the cache) once the transcript is
  // known. Both reads above are cached on disk, so a warm hit stays cheap.
